...
object ResultHandlerSpec extends BaseRunnableSpec({
  suite("ResultHandler") (
    testM("An IncrementedTo(1) event is logged correctly") {
      ...
      assertM((createResultHandler >>= (processResult(IncrementedTo(1)).provide)) *> getLastLogEntry(Info), equalTo(expected))
    },
    testM("A ComputedTotal(0) event is logged correctly") {
      val expected = "The current value has been computed to '0'"
      assertM((createResultHandler >>= processResult(ComputedTotal(0)).provide) *> getLastLogEntry(Info), equalTo(expected))
    }
  )
})
