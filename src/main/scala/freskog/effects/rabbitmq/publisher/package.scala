...
package object publisher {
  ...
  def publishTo(exchange: String, topology: Declaration): ZIO[Publisher, Nothing, String => IO[IOException, Unit]] =
    ZIO.accessM[Publisher](_.publisher.publishTo(exchange, topology))

  def publishConfirmsTo(exchange: String, topology: Declaration): ZIO[Publisher, Nothing, String => IO[IOException, Unit]] =
