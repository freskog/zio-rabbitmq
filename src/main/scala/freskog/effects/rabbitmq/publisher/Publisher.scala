...
import com.rabbitmq.client.{AMQP, Channel, ConfirmListener, ShutdownListener, ShutdownSignalException}
...
object Publisher extends Serializable {
  ...
  def publishWithConfirmsFiber(exchange: String, topology: Declaration, queue: Queue[(Promise[IOException, Unit],String)]): ZIO[PublisherEnv, Nothing, Fiber[IOException, Unit]] =
  ...
  def publishMsg(channel:Channel, ex:String, queue:Queue[(Promise[IOException, Unit],String)], inflight: RefM[Map[Long, Promise[IOException, Unit]]]): ZIO[AdminClient, IOException, Promise[IOException, Unit]] =
    for {
  ...
  def publishFiber(exchange: String, topology:Declaration, queue: Queue[String]): ZIO[PublisherEnv, Nothing, Fiber[IOException, Unit]] =
    createManagedChannel(exchange).use { channel =>
      for {
        _ <- createTopology(topology)
  ...
  def makeConfirmListener(channel: Channel, rts: Runtime[Any], inflight: RefM[Map[Long, Promise[IOException, Unit]]]): ConfirmListener =
    new ConfirmListener {
  ...
  def makeShutdownListener(queue:Queue[(Promise[IOException, Unit],String)], rts:Runtime[Any], inflight:RefM[Map[Long,Promise[IOException,Unit]]]): ShutdownListener =
    (cause: ShutdownSignalException) => rts.unsafeRun(
      for {
        m <- inflight.get
